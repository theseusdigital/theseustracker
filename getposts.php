	...
	if(isset($_POST['branddata'])){
		...
		if($days >= 1){
		    ...
		    if($platformid == 4){
		    	...
		    	$handleid = $brandhandle['id'];
		    	$handlename = $brandhandle['name'];
		  		$query = "select postid,caption,likes,comments,views,postimg,url,published,posttype from tracker_instagramhandlepost where handle_id = '$handleid' and published >= '$since 00:00:00' and published <= '$until 23:59:59' and caption like '%$searchvalue%' order by published desc limit 3";
			    $result = mysql_query($query);
			    $pagerecords = mysql_numrows($result);
			    if($pagerecords > 0){
