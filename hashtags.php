...
session_start();
    ...
    if(isset($_SESSION['hashtagplatform']))
    {
...
$(document).ready(function(){
    ...
    platform_map = {"Facebook":1,"Twitter":2};
    platformname = <?php echo json_encode($platformname); ?>;
    platformid = platform_map[platformname];
    $("#platforms option[id='"+platformid+"']").attr('selected', "selected");
    $("#platforms").selectmenu({
      select: function( event, ui ) {
        platformname = this.value;
        platformid = platform_map[platformname];
        Generate();
      }
    });
